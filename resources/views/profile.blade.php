...
@section('content')
    <div style="max-width: 560px;">
        <h1 style="font-size: 24px; font-weight: 700; color: var(--text-primary, #2D0A0A); margin: 0 0 24px;">{{ __('profile.title') }}</h1>
        ...
        <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; padding: 20px; margin-bottom: 24px;">
            <div style="margin-bottom: 16px;">
                <label style="display: block; font-size: 12px; font-weight: 500; color: var(--text-tertiary, #7A5050); margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.04em;">
                    {{ __('profile.email') }}
                </label>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <span style="font-size: 14px; color: var(--text-primary, #2D0A0A);" dir="ltr">{{ $user->email }}</span>
                    <span style="font-size: 11px; color: var(--text-tertiary, #7A5050);">({{ __('profile.managed_by_google') }})</span>
                </div>
            </div>
        ...
        <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; padding: 20px;">
            <form method="POST" action="{{ route('profile.update') }}">
                ...
                <div style="margin-bottom: 16px;">
                    <label for="name" style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 6px;">
                        {{ __('profile.name') }}
                    </label>
                ...
                <div style="margin-bottom: 20px;">
                    <label for="preferred_locale" style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 6px;">
                        {{ __('profile.preferred_locale') }}
                    </label>
                    <select id="preferred_locale" name="preferred_locale"
                            style="width: 100%; padding: 10px 12px; border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; font-size: 14px; color: var(--text-primary, #2D0A0A); outline: none; box-sizing: border-box; background: var(--surface-card, #FFFFFF);"
