<header style="background: #FFFFFF; border-bottom: 1px solid var(--border-default, #E7E5E4); height: 56px; display: flex; align-items: center; padding: 0 16px; gap: 12px; position: sticky; top: 0; z-index: 40;">
    {{-- Firm name --}}
    <div style="font-weight: 600; font-size: 16px; color: var(--text-primary, #2D0A0A); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0; max-width: 200px;">
    ...
    <button
        ...
        style="display: flex; align-items: center; gap: 8px; padding: 6px 12px; border: 1px solid var(--border-default, #E7E5E4); border-radius: 6px; background: var(--surface-page, #FAFAF9); color: var(--text-tertiary, #7A5050); font-size: 13px; cursor: pointer; min-width: 200px;"
        title="{{ __('shell.search') }}"
    >
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
        <span>{{ __('shell.search_placeholder') }}</span>
        <kbd style="margin-inline-start: auto; font-size: 11px; padding: 1px 5px; border: 1px solid var(--border-default, #E7E5E4); border-radius: 3px; background: #FFFFFF; color: var(--text-tertiary, #7A5050);">⌘K</kbd>
    </button>
    ...
    <div x-data="{ open: false }" style="position: relative;">
        ...
        >
            <a href="/app/matters/create" style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; color: var(--text-primary, #2D0A0A); text-decoration: none; border-radius: 4px;" onmouseover="this.style.background='var(--surface-card-hover, #F5F5F4)'" onmouseout="this.style.background='transparent'">
            ...
            <a href="/app/contacts/create" style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; color: var(--text-primary, #2D0A0A); text-decoration: none; border-radius: 4px;" onmouseover="this.style.background='var(--surface-card-hover, #F5F5F4)'" onmouseout="this.style.background='transparent'">
            ...
            <a href="/app/tasks/create" style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; color: var(--text-primary, #2D0A0A); text-decoration: none; border-radius: 4px;" onmouseover="this.style.background='var(--surface-card-hover, #F5F5F4)'" onmouseout="this.style.background='transparent'">
    ...
    <div x-data="{ open: false }" style="position: relative;">
        ...
        @else
            ...
            <button
                @click="open = !open"
                style="display: flex; align-items: center; gap: 4px; padding: 6px 10px; border: 1px solid var(--border-default, #E7E5E4); border-radius: 6px; background: #FFFFFF; font-size: 13px; color: var(--text-secondary, #4A2020); cursor: pointer;"
            ...
            >
                <div style="padding: 8px 12px; font-size: 11px; font-weight: 600; color: var(--text-tertiary, #7A5050); text-transform: uppercase; letter-spacing: 0.08em;">
                ...
                @forelse ($recentMatters as $matter)
                    <button
                        ...
                        @click="open = false"
                        style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; color: var(--text-primary, #2D0A0A); background: transparent; border: none; width: 100%; text-align: start; cursor: pointer; border-radius: 4px;"
                ...
                @empty
                    <div style="padding: 8px 12px; font-size: 13px; color: var(--text-tertiary, #7A5050);">
    ...
    <button
        wire:click="openChat"
        style="display: flex; align-items: center; justify-content: center; width: 36px; height: 36px; border: 1px solid var(--border-default, #E7E5E4); border-radius: 6px; background: #FFFFFF; color: var(--text-tertiary, #7A5050); cursor: pointer;"
        title="{{ __('shell.chat') }}"
    >
    ...
    <div x-data="{ open: false }" style="position: relative;">
        <button
            @click="open = !open"
            style="display: flex; align-items: center; justify-content: center; width: 36px; height: 36px; border: 1px solid var(--border-default, #E7E5E4); border-radius: 6px; background: #FFFFFF; color: var(--text-tertiary, #7A5050); cursor: pointer; position: relative;"
        ...
        >
            <div style="padding: 12px 16px; border-bottom: 1px solid var(--border-default, #E7E5E4); font-weight: 600; font-size: 14px; color: var(--text-primary, #2D0A0A);">
                {{ __('shell.notifications') }}
            </div>
            <div style="padding: 24px 16px; text-align: center; color: var(--text-tertiary, #7A5050); font-size: 13px;">
    ...
    <div x-data="{ open: false }" style="position: relative;">
        ...
        >
            ...
            @endif
            <span style="font-size: 13px; color: var(--text-primary, #2D0A0A); font-weight: 500; display: none;" class="sm:inline">{{ $user->name }}</span>
        </button>
        ...
        >
            ...
            <div style="padding: 8px 12px; border-bottom: 1px solid var(--border-default, #E7E5E4); margin-bottom: 4px;">
                <div style="font-size: 13px; font-weight: 600; color: var(--text-primary, #2D0A0A);">{{ $user->name }}</div>
                <div style="font-size: 12px; color: var(--text-tertiary, #7A5050);">{{ $user->email }}</div>
            </div>

            <a href="{{ route('profile') }}" style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; color: var(--text-primary, #2D0A0A); text-decoration: none; border-radius: 4px;" onmouseover="this.style.background='var(--surface-card-hover, #F5F5F4)'" onmouseout="this.style.background='transparent'">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                {{ __('shell.profile') }}
            </a>

            @if ($user->canAccessPanel(\Filament\Facades\Filament::getPanel('app')))
                <a href="/app" style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; color: var(--text-primary, #2D0A0A); text-decoration: none; border-radius: 4px;" onmouseover="this.style.background='var(--surface-card-hover, #F5F5F4)'" onmouseout="this.style.background='transparent'">
            ...
            {{-- Locale switcher --}}
            <a href="{{ url('/locale/' . (app()->getLocale() === 'ar' ? 'en' : 'ar')) }}" style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; color: var(--text-primary, #2D0A0A); text-decoration: none; border-radius: 4px;" onmouseover="this.style.background='var(--surface-card-hover, #F5F5F4)'" onmouseout="this.style.background='transparent'">
    ...
    >
        ...
        <div style="position: relative; background: #FFFFFF; border-radius: 12px; box-shadow: var(--shadow-xl); width: 100%; max-width: 560px; overflow: hidden;">
            <div style="display: flex; align-items: center; gap: 12px; padding: 16px; border-bottom: 1px solid var(--border-default, #E7E5E4);">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--text-tertiary, #7A5050)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
                <input
                    x-ref="searchInput"
                    type="text"
                    placeholder="{{ __('shell.search_full_placeholder') }}"
                    style="flex: 1; border: none; outline: none; font-size: 16px; color: var(--text-primary, #2D0A0A); background: transparent;"
                >
            </div>
            <div style="padding: 32px 16px; text-align: center; color: var(--text-tertiary, #7A5050); font-size: 14px;">
