...
<div>
    ...
    <div style="margin-bottom: 24px;">
        <a href="/matters" style="font-size: 13px; color: var(--text-tertiary, #7A5050); text-decoration: none; display: inline-flex; align-items: center; gap: 4px; margin-bottom: 12px;">
        ...
        <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 16px;">
            <div style="flex: 1;">
                <h1 style="font-size: 24px; font-weight: 700; color: var(--text-primary, #2D0A0A); margin: 0 0 8px;">{{ $matter->title }}</h1>
                <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                    ...
                    @if ($matter->client)
                        <span style="font-size: 13px; color: var(--text-secondary, #4A2020);">{{ $matter->client->display_name }}</span>
                    @endif
                    @if ($matter->practice_area)
                        <span style="font-size: 13px; color: var(--text-tertiary, #7A5050);">{{ $matter->practice_area->label() }}</span>
                    @endif
                </div>
            </div>
            @if (!$editing)
                <button wire:click="startEditing"
                    style="padding: 8px 16px; background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); cursor: pointer; display: flex; align-items: center; gap: 6px; flex-shrink: 0;">
            ...
            <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; padding: 24px;">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    ...
                    <div style="grid-column: span 2;">
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matters_form_title') }}</label>
                    ...
                    {{-- Client --}}
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matters_form_client') }}</label>
                    ...
                    {{-- Status --}}
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matters_form_status') }}</label>
                    ...
                    {{-- Practice Area --}}
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matters_form_practice_area') }}</label>
                    ...
                    {{-- Internal Reference --}}
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matter_internal_ref') }}</label>
                    ...
                    {{-- Stage --}}
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matter_stage') }}</label>
                    ...
                    {{-- Opened At --}}
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matter_opened') }}</label>
                    ...
                    {{-- Closed At --}}
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matter_closed') }}</label>
                    ...
                    {{-- Description --}}
                    <div style="grid-column: span 2;">
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matter_description') }}</label>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                ...
                @foreach ([
                    ...
                    @if ($value)
                        <div style="display: flex; justify-content: space-between; padding: 6px 0; font-size: 13px;">
                            <span style="color: var(--text-tertiary, #7A5050);">{{ $label }}</span>
                            <span style="color: var(--text-primary, #2D0A0A); font-weight: 500;">{{ $value }}</span>
                        </div>
                    @endif
                @endforeach
            </div>
            ...
            <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; padding: 20px;">
                ...
                @forelse ($matter->counterparties as $cp)
                    <div style="padding: 6px 0; font-size: 13px; color: var(--text-primary, #2D0A0A);">
                        {{ $cp->display_name }}
                    </div>
                @empty
                    <p style="font-size: 13px; color: var(--text-tertiary, #7A5050);">{{ __('shell.matter_no_counterparties') }}</p>
                @endforelse
            </div>
            ...
            @if ($matter->description)
                <div style="grid-column: span 2; background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; padding: 20px;">
                    ...
                    <p style="font-size: 14px; color: var(--text-secondary, #4A2020); line-height: 1.6; margin: 0; white-space: pre-wrap;">{{ $matter->description }}</p>
                </div>
            @endif
        </div>
        @endif
                ...
                <a href="/matters/{{ $matter->id }}/documents/{{ $doc->id }}"
                   ...
                   onmouseover="this.style.background='var(--surface-card-hover, #F5F5F4)'" onmouseout="this.style.background='transparent'">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <div style="font-size: 14px; font-weight: 500; color: var(--text-primary, #2D0A0A);">{{ $doc->title }}</div>
                        <div style="font-size: 12px; color: var(--text-tertiary, #7A5050);">{{ $doc->updated_at->diffForHumans() }}</div>
                    </div>
                </a>
            @empty
                <div style="padding: 40px; text-align: center; color: var(--text-tertiary, #7A5050); font-size: 13px;">{{ __('shell.matter_no_documents') }}</div>
            @endforelse
        </div>
    ...
    @elseif ($activeTab === 'tasks')
        {{-- Tasks --}}
        <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; overflow: hidden;">
            @php $priorityColors = ['urgent' => '#DC2626', 'high' => '#F59E0B', 'medium' => '#2563EB', 'normal' => '#7A5050', 'low' => '#A89090']; @endphp
            @forelse ($matter->tasks as $task)
                <div style="display: flex; align-items: center; gap: 10px; padding: 12px 16px; border-bottom: 1px solid var(--border-default, #E7E5E4);">
                    <span style="width: 8px; height: 8px; border-radius: 9999px; background: {{ $priorityColors[$task->priority?->value ?? 'normal'] ?? '#7A5050' }}; flex-shrink: 0;"></span>
                    <div style="flex: 1; min-width: 0;">
                        <div style="font-size: 14px; font-weight: 500; color: var(--text-primary, #2D0A0A);">{{ $task->title }}</div>
                        <div style="font-size: 12px; color: var(--text-tertiary, #7A5050); display: flex; gap: 8px;">
            ...
            @empty
                <div style="padding: 40px; text-align: center; color: var(--text-tertiary, #7A5050); font-size: 13px;">{{ __('shell.matter_no_tasks') }}</div>
            @endforelse
        </div>
        ...
        <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; overflow: hidden;">
            @forelse ($matter->hearings->sortByDesc('hearing_date') as $hearing)
                <div style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-bottom: 1px solid var(--border-default, #E7E5E4);">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <div style="font-size: 14px; font-weight: 500; color: var(--text-primary, #2D0A0A);">
            ...
            @empty
                <div style="padding: 40px; text-align: center; color: var(--text-tertiary, #7A5050); font-size: 13px;">{{ __('shell.matter_no_hearings') }}</div>
            @endforelse
        </div>
        ...
        <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; overflow: hidden;">
            @forelse ($matter->aiDocumentGenerations as $gen)
                <div style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-bottom: 1px solid var(--border-default, #E7E5E4);">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <div style="font-size: 14px; font-weight: 500; color: var(--text-primary, #2D0A0A);">{{ $gen->template_key }}</div>
                        <div style="font-size: 12px; color: var(--text-tertiary, #7A5050);">
            ...
            @empty
                <div style="padding: 40px; text-align: center; color: var(--text-tertiary, #7A5050); font-size: 13px;">{{ __('shell.matter_no_ai') }}</div>
            @endforelse
        </div>
        ...
        <div style="background: var(--surface-card, #FFFFFF); border: 1px solid var(--border-default, #E7E5E4); border-radius: 8px; overflow: hidden;">
            @forelse ($matter->matterLawyers as $ml)
                <div style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-bottom: 1px solid var(--border-default, #E7E5E4);">
                    ...
                    <div style="flex: 1;">
                        <div style="font-size: 14px; font-weight: 500; color: var(--text-primary, #2D0A0A);">{{ $ml->user?->name }}</div>
                        <div style="font-size: 12px; color: var(--text-tertiary, #7A5050);">{{ $ml->user?->email }}</div>
                    </div>
                    <span style="font-size: 11px; font-weight: 600; padding: 3px 10px; border-radius: 9999px; text-transform: uppercase; letter-spacing: 0.04em;
                        {{ $ml->role->value === 'lead' ? 'background: var(--color-brand-50, #FDF2F2); color: var(--color-brand-700, #330000);' : 'background: #F5F5F4; color: #5C3535;' }}">
            ...
            @empty
                <div style="padding: 40px; text-align: center; color: var(--text-tertiary, #7A5050); font-size: 13px;">{{ __('shell.matter_no_team') }}</div>
            @endforelse
        </div>
                ...
                <div style="position: relative; background: #FFFFFF; border-radius: 12px; box-shadow: var(--shadow-xl); width: 100%; max-width: 440px; padding: 24px;">
                    ...
                    <form wire:submit="addMember">
                        <div style="margin-bottom: 14px;">
                            <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matter_select_member') }}</label>
                        ...
                        <div style="margin-bottom: 14px;">
                            <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.matter_member_role') }}</label>
