            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                <thead>
                    <tr style="background: var(--surface-subtle, #F5F5F4); border-bottom: 1px solid var(--border-default, #D6D3D1);">
                        <th style="padding: 12px 16px; text-align: start; font-weight: 600; color: var(--text-secondary, #5C3535);">{{ __('shell.col_display_name') }}</th>
                        <th style="padding: 12px 16px; text-align: start; font-weight: 600; color: var(--text-secondary, #5C3535);">{{ __('shell.col_type') }}</th>
                        <th style="padding: 12px 16px; text-align: start; font-weight: 600; color: var(--text-secondary, #5C3535);">{{ __('shell.col_email') }}</th>
                        <th style="padding: 12px 16px; text-align: start; font-weight: 600; color: var(--text-secondary, #5C3535);">{{ __('shell.col_phone') }}</th>
                        <th style="padding: 12px 16px; text-align: start; font-weight: 600; color: var(--text-secondary, #5C3535);">{{ __('shell.col_flags') }}</th>
                            ...
                            <td style="padding: 12px 16px;">
                                @php
                                    $typeBadge = match($contact->type) {
                                        ...
                                        default => ['bg' => '#F5F5F4', 'text' => '#5C3535', 'label' => $contact->type ?? '—'],
                                    };
                                @endphp
                            ...
                            <td style="padding: 12px 16px;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    @if ($contact->is_client)
                                        <span style="display: inline-flex; align-items: center; gap: 4px; font-size: 12px; color: var(--text-secondary, #5C3535);" title="{{ __('shell.flag_client') }}">
                                    ...
                                    @if ($contact->is_counterparty)
                                        <span style="display: inline-flex; align-items: center; gap: 4px; font-size: 12px; color: var(--text-secondary, #5C3535);" title="{{ __('shell.flag_counterparty') }}">
                                    ...
                                    @if (!$contact->is_client && !$contact->is_counterparty)
                                        <span style="color: var(--text-muted, #A89090);">—</span>
                                    @endif
                                </div>
                            </td>
        ...
        <div style="position: relative; background: #FFFFFF; border-radius: 12px; box-shadow: var(--shadow-xl); width: 100%; max-width: 600px; max-height: 80vh; overflow-y: auto; padding: 24px;">
            ...
            <form wire:submit="save">
                <div style="margin-bottom: 14px;">
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.label_type') }}</label>
                ...
                @if ($formType === 'person')
                    <div style="margin-bottom: 14px;">
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.label_first_name') }}</label>
                    ...
                    <div style="margin-bottom: 14px;">
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.label_last_name') }}</label>
                ...
                @else
                    <div style="margin-bottom: 14px;">
                        <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.label_organization_name') }}</label>
                ...
                @endif
                <div style="margin-bottom: 14px;">
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.label_email') }}</label>
                ...
                <div style="margin-bottom: 14px;">
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary, #4A2020); margin-bottom: 4px;">{{ __('shell.label_phone') }}</label>
