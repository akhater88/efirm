...
- 100% of color values referenced in Waves W2–W17 come from the brand token system (zero raw hex codes in component code, enforced via PR lint rule)
- 100% of typography declarations in Waves W2–W17 use named font-family tokens (zero ad-hoc font stacks)
...
- Zero engineer-agent clarification requests on color or type during Waves W2–W17 (measures spec completeness)

**Business Value:** A single brand foundation is the lowest-cost moment to make brand decisions. Made now: one Wave. Made distributed across 16 Waves: rework on every Wave plus a final consolidation Wave. The cost differential is roughly 12–16x. Additionally, self-hosting fonts (rather than Google Fonts CDN) addresses Jordan PDPL Article 13 third-party data-flow scrutiny by eliminating Google Fonts' IP-logging side effect — a small but real regulatory hygiene win.
...
| Wave | Flow | Consumes |
|---|---|---|
| W2–W3 | F-DASH-01.5 (Top chrome) | All color, type, spacing tokens; logo (mark + reversed); tier badge styling |
| W4 | F-DASH-01.6 (Left sidebar) | `surface-sidebar*` aliases; logo-reversed; RTL logical properties |
| W5 | F-DASH-01.7 (Hero + AI placeholder) | `display-*` scale (Latin only); `brand-500` accents |
| W6 | F-DASH-01.8 (Widget grid + card) | `surface-card`, `border-default`, `shadow-sm`, `rounded-lg` |
| W7–W11 | F-DASH-01.9 through .13 (Widgets) | Body + heading scale; semantic tokens; empty/loading/error patterns |
| W12 | F-DASH-01.14 (Service Log feed) | Same as W7–W11; pending Khaldoun input on entity semantics |
| W13 | F-DASH-01.15 (Quick Links) | `brand-500` icon tinting; `text-link*` |
| W14–W17 | F-DASH-01.16 (Polish + QA) | All tokens; runs final contrast + RTL audit |
